...
export class ClientProjectDetailsComponent {
  ...
  getBuildStatusClasses(status: any): string {
    if (status === 1 || String(status) === '1') return 'bg-emerald-100 text-emerald-700';
    if (status === 0 || String(status) === '0') return 'bg-sky-100 text-sky-700';
  ...
  getPaymentText(status: any): string {
    return status === 1 || String(status) === '1' ? 'Paid' : 'Pending';
  }

  getPaymentClasses(status: any): string {
    return status === 1 || String(status) === '1'
      ? 'bg-emerald-100 text-emerald-700'
      : 'bg-amber-100 text-amber-700';
  }
  ...
  getLatestTemplate(): any {
    return this.templateHistory[0] || null;
  }
